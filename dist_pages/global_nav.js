document.addEventListener('DOMContentLoaded', () => {
    ...
    style.innerHTML = `
        /* CORPORATE INTELLIGENCE DARK NAV */
        #global-nav {
            position: fixed;
        ...
        .global-nav-brand img {
            height: 42px; border-radius: 10px; border: 1px solid rgba(56, 189, 248, 0.3);
        ...
        .global-nav-links a {
            color: #94a3b8; text-decoration: none; font-size: 0.9rem; font-weight: 600;
            text-transform: uppercase; letter-spacing: 1px; transition: all 0.3s ease;
            display: flex; align-items: center; gap: 0.6rem;
            position: relative;
            padding: 0.5rem 0;
        }
        .global-nav-links a i {
            font-size: 1.1rem;
            opacity: 0.7;
        ...
        .global-nav-links a:hover i, .global-nav-links a.active i {
            color: #38bdf8;
            opacity: 1;
    ...
    const ext = window.location.protocol === "file:" ? ".html" : "";
    const header = document.createElement('header');
    header.id = 'global-nav';
    header.innerHTML = `
        <a href="/landing${ext}" class="global-nav-brand">
            <img src="/logo_nemesis.jpeg" alt="NEMESIS Logo">
            <span>NEMESIS</span>
        </a>
        <nav class="global-nav-links">
            <a href="/landing${ext}" class="${isLanding ? 'active' : ''}"><i class="fa-solid fa-house"></i> Home</a>
            <a href="/tracer${ext}" class="${isTracer ? 'active' : ''}"><i class="fa-solid fa-project-diagram"></i> Tracer</a>
            <a href="/nemesis_id${ext}" class="${isID ? 'active' : ''}"><i class="fa-solid fa-id-card-clip"></i> Nemesis ID</a>
            <a href="/darknet_portal${ext}" class="${path.includes('darknet_portal') ? 'active' : ''}"><i class="fa-solid fa-user-secret"></i> Darknet</a>
            <a href="/admin${ext}" class="${isAdmin ? 'active' : ''}"><i class="fa-solid fa-server"></i> Admin / C2</a>
            <a href="/audit_report${ext}" class="${isAudit ? 'active' : ''}"><i class="fa-solid fa-shield-halved"></i> Master Audit</a>
            <a href="/recovery_framework${ext}" class="${isFramework ? 'active' : ''}"><i class="fa-solid fa-scale-balanced"></i> Framework</a>
        </nav>
    `;
